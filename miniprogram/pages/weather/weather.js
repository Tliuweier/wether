...
Page({
  ...
  bindPoster:function(){
    let locationString = this.data.locationString;
    let temperature = this.data.now.temperature
    let code = this.data.now.code
    let text = this.data.now.text
    wx.navigateTo({
      url: `../poster/poster?location=${locationString}&text=${text}&code=${code}&temperature=${temperature}`
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
